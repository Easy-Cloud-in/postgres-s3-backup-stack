...
validate_env() {
    ...
    # Ensure S3_BUCKET matches the expected format for client
    if [ -z "${S3_BUCKET}" ]; then
        log "S3_BUCKET is not set, constructing from S3_BUCKET_PREFIX and CLIENT_NAME"
        export S3_BUCKET="${S3_BUCKET_PREFIX}-${CLIENT_NAME}"
    fi
}
...
# Set the correct WALG_S3_PREFIX for the backup container
log "Setting correct S3 bucket configuration..."
docker compose exec backup bash -c "export WALG_S3_PREFIX=${WALG_S3_PREFIX}"
check_error "Failed to set S3 bucket configuration"
...
if [ -z "$BACKUPS" ]; then
    log "Error: No backups found in S3 bucket ${S3_BUCKET}"
    exit 1
fi
