...
create_s3_structure() {
    ...
    # Use the existing S3_BUCKET environment variable
    ...
    # S3_BUCKET is already set in the environment
...
# Function to update .env with client name and S3 bucket
update_env_with_client() {
    ...
    # Update S3_BUCKET based on client name
    local bucket_name="htxpp-backup-${client_name}"
    if ! grep -q "^S3_BUCKET=" .env; then
        echo "S3_BUCKET=$bucket_name" >> .env
    else
        sed -i "s/^S3_BUCKET=.*/S3_BUCKET=$bucket_name/" .env
    fi
}
...
main() {
    ...
    create_s3_structure "$CLIENT_NAME" "$AWS_REGION"
